...
- Notify-and-tap note opening for a headless board host (`contrib/`): `ktui-board` now sets `KANBAN_TUI_NOTE_OPEN_CMD` to the new `ktui-notify-note`, which posts the `obsidian://` link to an ntfy topic; the user taps the notification on the device in hand and Obsidian opens there. New `ktui-notify-tap` is the desktop-side subscriber hook (notification with an Open button, only `obsidian://` honoured). Replaces the reverse SSH hop, so the board host no longer needs a key for any desktop and never connects into one.
