...
- `KANBAN_TUI_NOTE_OPEN_CMD`: command used by `o` instead of `xdg-open`, with `{uri}` substituted (appended if absent). Lets a TUI running over SSH open the note on the machine you are sitting at, e.g. `ssh me@desktop env WAYLAND_DISPLAY=wayland-1 xdg-open \'{uri}\'`. Without it and without a `DISPLAY`/`WAYLAND_DISPLAY`, `o` copies the `obsidian://` URI to the clipboard and says so instead of silently failing. When the command runs through a remote shell (ssh), quote the placeholder as `\'{uri}\'`: the URI contains `?` and `&`.
- Notify-and-tap note opening for a headless board host. When `o` finds no display and no `KANBAN_TUI_NOTE_OPEN_CMD` but an ntfy token file (`~/.config/ntfy/ktui-token`), it posts the `obsidian://` link to an ntfy topic as a notification; the user taps it on the device in hand and Obsidian opens there. The host only connects outward and holds no key for any desktop, replacing the earlier reverse-SSH hop. Server and topic via `KTUI_NTFY_URL` / `KTUI_NTFY_TOPIC`. Everything ships in the package as console scripts, so `uv tool install` is the whole deploy: `ktui-board` (TUI with board-host defaults), `ktui-notify-note` (send a link by hand, for testing), `ktui-notify-tap` (desktop hook for `ntfy subscribe`: notification with an Open button, only `obsidian://` honoured) and `ktui-open-uri` (open a URI in this desktop session). The `contrib/` shell scripts are gone.
- `o` now runs the note command in a worker and reports its result: a non-zero exit or timeout shows an error toast with the command's last stderr line instead of an unconditional "Opening note". With a custom `KANBAN_TUI_NOTE_OPEN_CMD` the success toast reads "Note link sent".
