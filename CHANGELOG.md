...
- `KANBAN_TUI_NOTE_OPEN_CMD`: command used by `o` instead of `xdg-open`, with `{uri}` substituted (appended if absent). Lets a TUI running over SSH open the note on the machine you are sitting at, e.g. `ssh me@desktop env WAYLAND_DISPLAY=wayland-1 xdg-open {uri}`. Without it and without a `DISPLAY`/`WAYLAND_DISPLAY`, `o` copies the `obsidian://` URI to the clipboard and says so instead of silently failing.
